...
class ModernDatepicker extends Component {
  ...
  addDate(value, unit){
    ...
    if ((maxDate && date.isAfter(endDate)) || (minDate && date.isBefore(startDate))){
      if((maxDate && date.isAfter(endDate)) && maxDate.get('year') === date.get('year') && maxDate.get('month') === date.get('month')) {
        firstDateToBeSet = true;
      } else if((minDate && date.isBefore(startDate)) && minDate.get('year') === date.get('year') && minDate.get('month') === date.get('month')) {
