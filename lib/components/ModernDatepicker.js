...
class ModernDatepicker extends Component {
  ...
  handleDateChange(value, unit) {
    ...
    // const defaultDate = moment(this.state.dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY').isBetween( minDate ? moment(minDate, format || 'DD-MM-YYYY').subtract(1, 'days') : moment().subtract(1, 'days'), maxDate ? moment(maxDate, format || 'DD-MM-YYYY') : moment(moment().add(1, 'days'), format || 'DD-MM-YYYY')) ? moment().format(format || 'DD-MM-YYYY') : maxDate && moment(maxDate, format || 'DD-MM-YYYY').isSameOrAfter(moment(this.state.dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY')) ? moment() : minDate && moment(minDate, format || 'DD-MM-YYYY').isSameOrBefore(moment(this.state.dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY')) ? moment() : maxDate ? moment(maxDate, format || 'DD-MM-YYYY').subtract(1, 'days').format(format || 'DD-MM-YYYY') : minDate ? moment(minDate, format || 'DD-MM-YYYY').add(1, 'days').format(format || 'DD-MM-YYYY') : moment().format(format || 'DD-MM-YYYY') ;
    const _date = moment(this.state.dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY');
  ...
  addDate(value, unit){
    ...
    // const defaultDate = moment(this.state.dateToEdit || moment(), format || 'DD-MM-YYYY').isBetween(minDate || moment().subtract(1, 'days'), maxDate || moment().add(1, 'days')) ? moment().format(format || 'DD-MM-YYYY') : maxDate && moment(maxDate, format || 'DD-MM-YYYY').isSameOrAfter(this.state.dateToEdit) ? moment().format(format || 'DD-MM-YYYY') : minDate && moment(minDate, format || 'DD-MM-YYYY').isSameOrBefore(this.state.dateToEdit) ? moment().format(format || 'DD-MM-YYYY') : maxDate ? moment(maxDate, format || 'DD-MM-YYYY').subtract(1, 'days').format(format || 'DD-MM-YYYY') : minDate ? moment(minDate, format || 'DD-MM-YYYY').add(1, 'days').format(format || 'DD-MM-YYYY') : moment().format(format || 'DD-MM-YYYY') ;
    // const defaultDate = moment(dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY').isBetween( minDate ? moment(minDate, format || 'DD-MM-YYYY').subtract(1, 'days') : moment().subtract(1, 'days'), maxDate ? moment(maxDate, format || 'DD-MM-YYYY') : moment(moment().add(1, 'days'), format || 'DD-MM-YYYY')) ? moment().format(format || 'DD-MM-YYYY') : maxDate && moment(maxDate, format || 'DD-MM-YYYY').isSameOrAfter(moment(dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY')) ? moment() : minDate && moment(minDate, format || 'DD-MM-YYYY').isSameOrBefore(moment(dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY')) ? moment() : maxDate ? moment(maxDate, format || 'DD-MM-YYYY').subtract(1, 'days').format(format || 'DD-MM-YYYY') : minDate ? moment(minDate, format || 'DD-MM-YYYY').add(1, 'days').format(format || 'DD-MM-YYYY') : moment().format(format || 'DD-MM-YYYY') ;
    const _date = moment(this.state.dateToEdit || moment().format(format || 'DD-MM-YYYY'), format || 'DD-MM-YYYY');
    const _minDate = moment(minDate, format || 'DD-MM-YYYY');
    const _maxDate = moment(maxDate, format || 'DD-MM-YYYY'); 
    const defaultDate = _date.isBetween( minDate ? _minDate.clone().subtract(1, 'days') : _date.clone().subtract(1, 'days'), maxDate ? _maxDate.clone().add(1, 'days') : _date.clone().add(1, 'days')) ? _date : maxDate && _maxDate.isSameOrAfter(_date) ? _date : minDate && _minDate.isSameOrBefore(_date)  ? _minDate : maxDate ? _maxDate.clone().subtract(1, 'days') : minDate ? _minDate.clone().add(1, 'days') : _date;
    ...
    const date = moment(dateToEdit || defaultDate, format || 'DD-MM-YYYY').clone().add(value, unit);
    ...
    if ((maxDate && date.isAfter(endDate)) || (minDate && date.isBefore(startDate))){
      if((maxDate && date.isAfter(endDate)) && moment(maxDate, format || 'DD-MM-YYYY').get('year') === date.get('year') && moment(maxDate, format || 'DD-MM-YYYY').get('month') === date.get('month')) {
        firstDateToBeSet = true;
      } else if((minDate && date.isBefore(startDate)) && moment(minDate, format || 'DD-MM-YYYY').get('year') === date.get('year') && moment(minDate, format || 'DD-MM-YYYY').get('month') === date.get('month')) {
  ...
  subDate(value, unit){
    ...
    const date = moment(dateToEdit || defaultDate, format || 'DD-MM-YYYY').clone().subtract(value, unit);
    const endDate = maxDate ? moment(maxDate, format || 'DD-MM-YYYY') : null;
    const startDate = minDate ? moment(minDate, format || 'DD-MM-YYYY') : null;
    if ((maxDate && date.isAfter(endDate))  || (minDate && date.isBefore(startDate))){
      if((maxDate && date.isAfter(endDate)) && moment(maxDate, format || 'DD-MM-YYYY').get('year') === date.get('year') && moment(maxDate, format || 'DD-MM-YYYY').get('month') === date.get('month')) {
  ...
  render() {
    ...
    return (
      <Container onBlur={(e) => this.onBlur(e)} >
        ...
        />}
        {showContainer && isValid && (this.checkAndReturnDate() === '' || moment(this.checkAndReturnDate(), format || 'DD-MM-YYYY').isValid()) &&  <CalendarContainer secondaryColor={secondaryColor}>
